...
Tous les serveurs de \fidit, quel que soit leur type, sont gérés de manière identique : avec \bsc{Linux}\footnote{Comme \bsc{Windows}, \bsc{Linux} est un système d'exploitation.}. Les employés travaillent directement sur le contenu des ces serveurs par le biais de connexions sécurisées dites \og SSH \fg\footnote{SSH : Secure SHell, une interface d'échanges distants, disponible uniquement en mode texte.}.
